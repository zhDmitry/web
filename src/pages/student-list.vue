<template>
  <div>
    <v-card>
      <v-card-title>
        <span class="h5 m-0">Студенти</span>
        <v-spacer></v-spacer>
      ...
      <v-card-actions v-if="canCreate">
        <v-dialog v-model="dialog" persistent width="500px">
          <v-btn color="primary" dark slot="activator">Зареєструвати студента</v-btn>
          <v-card>
            <v-card-text>
              <v-text-field label="ПІБ"></v-text-field>
              <v-text-field label="Університет"></v-text-field>
              <v-text-field label="Факультет"></v-text-field>
              <v-text-field label="Спеціальність"></v-text-field>
              <v-text-field label="Курс"></v-text-field>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  data() {
    return {
      ...
      dialog: false,
      canCreate: ['admin', 'methodist'].includes(this.$store.state.role)
    }
  }
}
</script>
